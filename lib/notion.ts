...
// 跨境访问 api.notion.com 偶发瞬时网络错误（ECONNRESET / 连接超时），
// 官方 client 的内置重试只覆盖 HTTP 状态码（429/5xx），不重试连接层错误。
// 这里包一层带指数退避的 fetch，把这类抖动挡在数据层内部，页面无需感知。
const RETRYABLE = /ECONNRESET|ETIMEDOUT|ECONNREFUSED|EPIPE|ENOTFOUND|EAI_AGAIN|socket hang up|network|fetch failed/i;
...
async function fetchWithRetry(
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      const delay = 300 * 2 ** i; // 300ms, 600ms, 1200ms…
      console.warn(`[notion] 网络抖动，${delay}ms 后重试 (${i + 1}/${attempts - 1})：${(err as Error).message}`);
      await sleep(delay);
    }
  }
  throw lastErr;
...
export const notion = new Client({ auth: token, fetch: fetchWithRetry });
